"""Static per-ticker characteristics used by the simulator."""
...
def profile_for(ticker: str) -> TickerProfile:
    ...
    Uses `hashlib`, not the built-in `hash()` — Python salts `hash()` for
    strings with a per-process random seed unless `PYTHONHASHSEED` is fixed,
    so the built-in would give a ticker a different price after every
    container restart. `hashlib.md5` is stable across processes and
    machines; it's used only as a stable string-to-number map, not as a
    security primitive.
    """
