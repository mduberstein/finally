...
The keys live in `.claude/settings.json` (project) or `/Users/mdub/SOFT-DEV/GitHubRepos/AIProjects/finally/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/mdub/SOFT-DEV/GitHubRepos/AIProjects/finally/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
