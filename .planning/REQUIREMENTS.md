...
- [x] **PORT-01**: User starts with $10,000 in virtual cash
- [x] **PORT-02**: User can buy shares of a ticker at the current market price — instant fill, no fees, no confirmation dialog
- [x] **PORT-03**: User can sell shares they own at the current market price — instant fill
- [x] **PORT-04**: Buying with insufficient cash is rejected with a clear error
...
- [x] **INFRA-03**: `docker-compose.yml` and `scripts/start_mac.sh` / `scripts/stop_mac.sh` (and Windows equivalents) let the user launch/stop the app with one command
